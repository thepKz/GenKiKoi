...
export const addressAutocomplete = async (req: Request, res: Response) => {
    try {
        ...
        if (!query) {
            return res.status(400).json({ message: "Vui lòng nhập địa chỉ để tìm kiếm" });
        }
        ...
        console.log(error);
        return res.status(500).json({ message: "Có lỗi xảy ra khi tìm kiếm địa chỉ" });
    }
};
...
export const calculateRoute = async (req: Request, res: Response) => {
    try {
        ...
        if (!address) {
            return res.status(400).json({ message: "Vui lòng nhập địa chỉ xuất phát" });
        }
        ...
        if (!originResponse.data?.[0]) {
            return res.status(404).json({ message: "Không tìm thấy địa chỉ xuất phát" });
        }
        ...
        const estimatedDurationMinutes = Math.round((distance / averageSpeedKmh) * adjustmentFactor * 60);
        ...
        console.log(error);
        return res.status(500).json({ message: "Có lỗi xảy ra khi tính toán tuyến đường" });
    }
};
