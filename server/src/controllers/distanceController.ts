...
const nominatimAxios = axios.create({
  ...
  headers: {
    "User-Agent":
      "GenKiKoi/1.0 (https://staginggenkikoi.netlify.app; maitanthepmrthep@gmail.com)",
...
export const calculateRoute = async (req: Request, res: Response) => {
  try {
    ...
    const originResponse = await nominatimAxios.get<NominatimResponse[]>(
      "/search",
      {
        params: {
          q: address,
    ...
    if (!originResponse.data || originResponse.data.length === 0) {
      return res
        .status(404)
        .json({ error: "Không tìm thấy địa chỉ xuất phát" });
    }
    ...
    const estimatedDurationHours =
      (distance / averageSpeedKmh) * adjustmentFactor;
